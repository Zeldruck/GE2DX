...
void ChipmunkShape::SetCollisionType(const std::string& _COLLISION_LAYER)
{
	if (!CollisionLayersManager::Instance().Exist(_COLLISION_LAYER))
	{
		printf("Collision layer, %f, does not exist.", _COLLISION_LAYER);
		return;
	}

	cpShapeSetCollisionType(m_handle, CollisionLayersManager::Instance().Get(_COLLISION_LAYER));
}

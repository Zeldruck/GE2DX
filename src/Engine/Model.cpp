...
Model::Model(std::shared_ptr<const SDLpp_texture> texture, std::vector<ModelVertex> vertices, std::vector<int> indices) :
...
void Model::Draw(SDLpp_renderer& renderer, const Transform& cameraTransform, const Transform& transform)
{
...
Model Model::LoadFromJSon(const nlohmann::json& doc)
{
	...
	// Textures
	std::shared_ptr<const SDLpp_texture> texture;
	if (!texturePath.empty())
